...
else
{
   ...
   ///////admin_login table///////////////////////////////////////////////// 
    ...
    $rs1x = mysqli_fetch_array($queryx);
      ...
      $reg_id=$rs1x["reg_id"];//reg_id of admin
...
<head>
  ...
  <title>Marwari College - D-Loker</title>
...
<li>
<a href="./dlocker.php">
  <i class="bi bi-circle"></i><span class="text-primary">D-Locker</span>
</a>
</li>
<li>
<a href="./resume.php">
  ...
  <div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">D-Locker</li>
      </ol>
    </nav>
  </div><!-- End Page Title -->
          ...
          <div class="card">
            <div class="card-body">
             ...
             <!-- regestration unique id -->         
                ...
                <!-- month -->
                <div class="col-md-4">
                  <label for="genre" class="form-label">Document Type</label>
                  <select id="genre" name="genre" class="form-select" aria-label="Default select example">
                    ...
                    <option value="Gradution Marksheet">Gradution Marksheet</option>                    
                ...
                <!-- picture  -->
                <div class="col-md-4">
                    <label for="pic" class="form-label">Upload</label>
                    <input name="pic" type="file" id="pic" class="form-control" autocomplete="off" required/>
                </div>

                <!-- random nu sno  -->
                <div class="col-md-4">
                    <label for="rand" class="form-label">Document Number</label>
                    <input name="rand" type="number" id="rand" class="form-control" placeholder="Document Serial No" autocomplete="off"/>
                </div>
                               
               
                <div class="text-center">
                  <button name="submit" style="border-radius: 20px; text-align: center;" type="submit" class="btn btn-outline-primary"><i class='bi bi-file-earmark-person'></i>   ADD</button>
...
<?php
              ...
              if(isset($_POST['submit']))
              {
                ...
                ///////////////////upload image////////////////////////
                $filename = $doc['name'];
                $filepath = $doc[ 'tmp_name'];
                $fileerror = $doc['error'];
                //$fileerror = $pic1['error'];
                $destfile = "./doc/".$filename;
                ...
                $res=mysqli_query($con,"SELECT * FROM `dlocker` WHERE type='$doc_type' AND reg='$reg_id'");
                    ...
                    echo"
                        <script>                
                            swal({
                                icon: 'error',
                                title: 'Aborted',
                                text: 'Document already Added!',
                                }).then(function() {
                                    window.location = 'dlocker.php';
                                });                        
                        </script>";
                }
                else
                {                                      
                    //insert data query
                    $sql="INSERT INTO `dlocker` (type, file, uid, reg) 
                    VALUES ('$doc_type','$filename','$uid','$reg_id')";
                    $result=$con->query($sql);
                    if($result)
                    {
                        echo"<script>                
            ...
            <div class="card-body">
              ...
              <div class="input-group">
                <div class="form-outline">
                    <input name="search" type="search" id="form1" class="form-control" placeholder="Document Name" autocomplete="off" required/>
                </div>
              ...
              <?php
                  ...
                  if(isset($_POST['ok']))
                  {
                    $sql="SELECT * FROM `dlocker` WHERE type='$_POST[search]' AND reg='$reg_id' ORDER BY sno DESC";
                    $result=$con->query($sql);
                    $n=1;
                    while($row=$result->fetch_assoc())
                    {
                        if($n==1)
                        echo"                                    
                        <a href='downlode.php?id=$row[id]' class='list-group-item list-group-item-action active' aria-current='true'>
                            <div class='d-flex w-100 justify-content-between'>
                            <h5 class='mb-1'>  ".$row['type']."</h5>
                            <small>  <b>Genre  </b>".$row['uid']."</small>
                            </div>          
                        ...
                        echo"                      
                        <a href='downlode.php?id=$row[id]' class='list-group-item list-group-item-action' >
                            <div class='d-flex w-100 justify-content-between'>
                            <h5 class='mb-1'>  ".$row['type']."</h5>
                            <small>  <b>Genre  </b>".$row['uid']."</small>
                            </div>          
                  ...
                  {
                        $sql="SELECT * FROM `dlocker` ORDER BY id DESC LIMIT 3";
                        $result=$con->query($sql);
                        $n=1;
                        while($row=$result->fetch_assoc())
                        {
