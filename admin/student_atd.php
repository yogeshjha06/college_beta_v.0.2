...
<head>
  ...
  <title>Marwari College - Ataindence</title>
...
<span>Log Out</span>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">Ataindence</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <div class="card">
    <div class="card-body">
        <h5 class="card-title">Search Month</h5>
        ...
        <form method="post">   
            ...
            <div class="input-group">
                ...
                <select style="border-top-right-radius: 0px;
                border-bottom-right-radius: 0px;
                border-top-left-radius: 20px;
                border-bottom-left-radius: 20px;" id="sem" name="find" class="form-select" aria-label="Default select example">
                ...
                if(isset($_POST['ok']))
                {
                      ...
                      //admin list table                    
                          $sql="SELECT * FROM  `attendance` WHERE month='$_POST[find]' AND reg='$reg_id'";
                          $result=$con->query($sql);
                          if($result->num_rows>0)
                          {
                            echo"
                                        ...
                                        while($row=$result->fetch_assoc())
                                        {
                                            if($row['attendance']<10)
                                            {
                                                echo"
                                                <tr>
                                                    <th  scope='row'><a href='' ><i class='bi bi-arrow-right-circle-fill'></a></i></th>
                                                    <td  style='overflow: hidden; text-overflow: ellipsis;'>".$row["month"]."</td>
                                                    <td  style='word-wrap: break-word; overflow: hidden; text-overflow: ellipsis;'>".$row["working_days"]."</td>
                                                    <td  ><span style='border-radius: 20px;background-color: #FF2400;' class='badge badge-primary'>".$row["attendance"]." Days</span></td>
                                                </tr>
                                                ";
                                            }
                                            else
                                            {
                                                echo"
                                                    <tr>
                                                        ...
                                                        <td  style='overflow: hidden; text-overflow: ellipsis;'>".$row["month"]."</td>
                                                        <td  style='word-wrap: break-word; overflow: hidden; text-overflow: ellipsis;'>".$row["working_days"]."</td>
                                                        <td  ><span style='border-radius: 20px;background-color: #0BDA51;' class='badge badge-success'>".$row["attendance"]." Days</span></td>
                                                    </tr>
                                                    ";
                                            }
                                        $no++;
                        ...
                        else
                        {
                            ...
                            <script>                
                                swal({
                                    icon: 'info',
                                    title:'Failed',
                                    text: 'Unable To Find Ataindence!',
                                    }).then(function() {
                                    window.location = 'student_atd.php';
                                    });                        
                                    </script>";
                        }
    ...
    <section class="section">
      <div class="row">
        ...
        <div class="card">
            <div class="card-body">
                <center>
              <h5 class="card-title">Ataindence List</h5>
              ...
              <table class="table table-hover">
                ...
                <?php
                //admin list table                    
                    $sql="SELECT * FROM  `attendance` WHERE reg='$reg_id' ORDER BY id DESC";
                    $result=$con->query($sql);
                    $no=1;
                    while($row=$result->fetch_assoc())
                    {
