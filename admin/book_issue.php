...
<head>
  ...
  <title>Marwari College - Book Issue</title>
        ...
        <ul id="forms-nav" class="nav-content" data-bs-parent="#sidebar-nav">
          ...
          <li>
            <a href="book_issue.php">
              <i class="bi bi-circle"></i><span class="text-primary">Issue Book</span>
            </a>
          </li>
          <li>
            ...
            <div class="card-body">
             ...
             <div class="col-md-6">
                ...
                <div class="col-md-6">
                  ...
                  <input name="issuedate" type="date" class="form-control" id="issuedate"  autocomplete="off" required/>
                </div>
                               
                <!-- book return date  -->
                <div class="col-md-6">
                  <label for="returndate" class="form-label">Date Of Return</label>
                  <input name="returndate" type="date" class="form-control" id="returndate"  autocomplete="off" required/>
                </div>
...
<?php
              ...
              if(isset($_POST['submit']))
              {
                ...
                $date_issue=$_POST['issuedate'];
                $date_return=$_POST['returndate'];
                $res1=mysqli_query($con,"SELECT * FROM `book_db` WHERE sno='$book_no' AND name='$book_name'");
                //check if book available
                $res=mysqli_query($con,"SELECT * FROM `book_db` WHERE sno='$book_no' AND status='1'");
                ...
                elseif(mysqli_num_rows ($res1) >0)
                {
                  ///////book list table///////////////////////////////////////////////// 
                    ...
                    $rs1x = mysqli_fetch_array($queryx);
                      ...
                      $new_qun=$book_qun-1;//new quantity

                    //update book list status         
                    if($new_qun==0)
                    {
                      //out of stock
                      $res=mysqli_query($con,"UPDATE `book_db` SET `status`='1', `qun`='$new_qun' WHERE sno='$book_no'");
                    }
                    else
                    {
                      //available book with new quantity
                      $res=mysqli_query($con,"UPDATE `book_db` SET `qun`='$new_qun' WHERE sno='$book_no'");
                    }
                    //////////////////////////////////////////////////////////////////////////////////////////

                    //insert data query
                    $sql="INSERT INTO `lib_record` (reg, date_issue, date_return, book_name, book_no) 
                    VALUES ('$stu_reg','$date_issue','$date_return','$book_name','$book_no')";
                    $result=$con->query($sql);
                    if($result)
                    {
                        echo"<script>                
                            swal({
                                icon: 'success',
                                title: 'Success',
                                text: 'Book has been issued to $stu_reg!',
                                }).then(function() {
                                    window.location = 'book_issue.php';
                                });                        
                                </script>";
                ...
                else
                {
                    ...
                    <script>                
                        swal({
                            icon: 'error',
                            title:'Failed',
                            text: 'Invalid request to issue book!',
                            }).then(function() {
                            window.location = 'book_issue.php';
                            });                        
                            </script>";
