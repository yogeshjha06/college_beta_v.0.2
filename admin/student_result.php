    ...
    <div class="card-body">
        ...
        <form method="post">   
            ...
            <div class="input-group">
                ...
                border-bottom-left-radius: 20px;"name="find" class="form-control" type="search" placeholder="Search Result" id="example-search-input4">                   
                </div>
                ...
                if(isset($_POST['ok']))
                {
                      ...
                      //admin list table                    
                          $sql="SELECT * FROM  `result_data` WHERE paper='$_POST[find]' AND reg_id='$reg_id'";
                          $result=$con->query($sql);
                          if($result->num_rows>0)
                          {
                            echo"
                                        ...
                                        while($row=$result->fetch_assoc())
                                        {
                                            if($row['result']<$row['pass'])
                                            {
                                                echo"
                                                <tr>
                                                    <th  scope='row'><a href='' ><i class='bi bi-arrow-right-circle-fill'></a></i></th>
                                                    <td  style='overflow: hidden; text-overflow: ellipsis;'>".$row["semester"]."</td>
                                                    <td  style='word-wrap: break-word; overflow: hidden; text-overflow: ellipsis;'>".$row["paper"]."</td>
                                                    <td  ><span style='border-radius: 20px;background-color: #FF2400;' class='badge badge-primary'>".$row["result"]." Fail</span></td>
                                                </tr>
                                                ";
                                            }
                                            else
                                            {
                                                echo"
                                                    <tr>
                                                        ...
                                                        <td  style='overflow: hidden; text-overflow: ellipsis;'>".$row["semester"]."</td>
                                                        <td  style='word-wrap: break-word; overflow: hidden; text-overflow: ellipsis;'>".$row["paper"]."</td>
                                                        <td  ><span style='border-radius: 20px;background-color: #0BDA51;' class='badge badge-success'>".$row["result"]." Pass</span></td>
                                                    </tr>
                                                    ";
                                            }
                                        $no++;
                        ...
                        else
                        {
                            ...
                            <script>                
                                swal({
                                    icon: 'info',
                                    title:'Failed',
                                    text: 'Unable To Find Results!',
                                    }).then(function() {
                                    window.location = 'student_result.php';
                                    });                        
                                    </script>";
                        }
        ...
        <div class="card">
            <div class="card-body">
              ...
              <table class="table table-hover">
                ...
                <?php
                //admin list table                    
                    $sql="SELECT * FROM  `result_data` WHERE reg_id='$reg_id' ORDER BY id DESC";
                    $result=$con->query($sql);
                    $no=1;
                    while($row=$result->fetch_assoc())
                    {
