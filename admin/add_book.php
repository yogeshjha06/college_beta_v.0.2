...
<head>
  ...
  <title>Marwari College - Add Book</title>
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        ...
        <ul id="forms-nav" class="nav-content" data-bs-parent="#sidebar-nav">
          ...
          <li>
            <a href="add_book.php">
              <i class="bi bi-circle"></i><span class="text-primary">Add Book</span>
            </a>
          </li>
          <li>
            ...
            <div class="card-body">
             ...
             <!-- regestration unique id -->
                <div class="col-md-6">
                  ...
                  <input name="book" type="text" class="form-control" id="book" placeholder="Book Name" autocomplete="off" required/>
                </div>             
                ...
                <!-- random nu sno  -->
                <div class="col-md-4">
                    <label for="rand" class="form-label">Serial No</label>
                    <input name="rand" type="number" id="rand" class="form-control" placeholder="Book Serial No" autocomplete="off" required/>
                </div>
                               
                <!-- quantity  -->
                <div class="col-md-4">
                  <label for="quantity" class="form-label">Quantity</label>
                  <input name="quantity" type="number" class="form-control" id="quantity" placeholder="Quantity Of Book" autocomplete="off" required/>
                </div>
               
                <div class="text-center">
                  <button name="submit" style="border-radius: 20px; text-align: center;" type="submit" class="btn btn-outline-primary"><i class='bi bi-journal-plus'></i>   Submit</button>
...
<?php
              ...
              if(isset($_POST['submit']))
              {
                ...
                ///////////////////upload image////////////////////////
                $filename = $pic1['name'];
                $filepath = $pic1[ 'tmp_name'];
                $fileerror = $pic1['error'];
                //$fileerror = $pic1['error'];
                $destfile = "./book/".$filename;
                ...
                $res=mysqli_query($con,"SELECT * FROM `book_db` WHERE name='$book' AND genre='$genre'");
                ...
                {                                      
                    //insert data query
                    $sql="INSERT INTO `book_db` (name, genre, qun, pic, sno) 
                    VALUES ('$book','$genre','$quantity','$filename','$rand')";
                    $result=$con->query($sql);
                    if($result)
                    {
                        echo"<script>                
            ...
            <div class="card-body">
              ...
              <div class="input-group">
                <div class="form-outline">
                    <input name="search" type="search" id="form1" class="form-control" placeholder="Book Name" autocomplete="off" required/>
                </div>
              ...
              <?php
                  ...
                  if(isset($_POST['ok']))
                  {
                    $sql="SELECT * FROM `book_db` WHERE name='$_POST[search]'";
                    $result=$con->query($sql);
                    $n=1;
                    while($row=$result->fetch_assoc())
                    {
                        if($n==1)
                        echo"                                    
                        <a href='./book/".$row['pic']."' class='list-group-item list-group-item-action active' aria-current='true'>
                            <div class='d-flex w-100 justify-content-between'>
                            <h5 class='mb-1'>  ".$row['name']."</h5>
                            <small>  <b>Genre  </b>".$row['genre']."</small>
                            </div>          
                            <p class='mb-1'> <b>Serial No   </b>".$row['sno']."</p>                  
                            <small> <b>Quantity  </b>".$row['qun']." </small>
                        </a>

                        ";
                        else
                        echo"                      
                        <a href='./book/".$row['pic']."' class='list-group-item list-group-item-action' >
                            <div class='d-flex w-100 justify-content-between'>
                            <h5 class='mb-1'>  ".$row['name']."</h5>
                            <small class='text-muted'>  <b>Genre  </b>".$row['genre']."</small>
                            </div>    
                            <p class='mb-1'> <b>Serial No   </b>".$row['sno']."</p>  
                            <small class='text-muted'> <b>Quantity  </b>".$row['qun']."</small>
                        </a>
                  ...
                  {
                        $sql="SELECT * FROM `book_db` ORDER BY id DESC LIMIT 3";
                        $result=$con->query($sql);
                        $n=1;
                        while($row=$result->fetch_assoc())
                        {
